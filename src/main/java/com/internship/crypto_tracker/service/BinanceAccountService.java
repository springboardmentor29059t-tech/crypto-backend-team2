...
public class BinanceAccountService {
    ...
    public void syncTradesForUser(Long userId, String apiKey, String apiSecret, String symbol, Exchange binanceExchange) {
        try {
            ...
            if (tradeList == null || tradeList.isEmpty()) return; 
            ...
            for (Map<String, Object> t : tradeList) {
                ...
                if (!tradeRepository.existsByUserIdAndAssetSymbolAndExecutedAtAndQuantity(
                    ...
                    trade.setUser(user);
                    trade.setExchange(binanceExchange); 
                    trade.setAssetSymbol(symbol);
                    trade.setSide((boolean) t.get("isBuyer") ? Trade.Side.BUY : Trade.Side.SELL);
                    trade.setQuantity(qty);
                    trade.setPrice(price);
                    trade.setFee(new BigDecimal(t.get("commission").toString()));
    ...
    private void updateCostBasis(Long userId, String symbol) {
        List<Trade> symbolTrades = tradeRepository.findByUserIdAndAssetSymbolOrderByExecutedAtAsc(userId, symbol);
